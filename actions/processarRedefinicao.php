...
if ($novaSenha !== $confirmarSenha) {
    header("Location: ../pages/auth/redefinirSenha.php?email=" . urlencode($email) . "&erro=senhas");
    exit;
}

/* Verifica o tamanho mínimo da senha */
if (strlen($novaSenha) < 8) {
    header("Location: ../pages/auth/redefinirSenha.php?email=" . urlencode($email) . "&erro=senhaP");
    exit;
}
...
if ($resultado->num_rows === 0) {
    ...
    header("Location: ../pages/auth/redefinirSenha.php?email=" . urlencode($email) . "&erro=codigo");
    exit;
}
...
if (strtotime($row['codigo_expira']) < time()) {
    header("Location: ../pages/auth/redefinirSenha.php?email=" . urlencode($email) . "&erro=expirado");
    exit;
}
